...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        # 输入: x (batch_size, in_channels, length), skip (batch_size, out_channels, length * 2)
...
class UNet1D(nn.Module):
    def __init__(self, input_dim, time_embed_dim=256, condition_dim=128):
        ...
        # 下采样路径
        self.down1 = DownBlock(64, 128)  # 下采样块1
        self.down2 = DownBlock(128, 256)  # 下采样块2
        self.down3 = DownBlock(256, 512)  # 下采样块3

        # 中间层
        self.mid_conv = ConvBlock(512, 1024)  # 中间卷积块

        # 上采样路径
        self.up1 = UpBlock(1024, 512)  # 上采样块1
        self.up2 = UpBlock(512, 256)  # 上采样块2
        self.up3 = UpBlock(256, 128)  # 上采样块3
    ...
    def forward(self, x):
        ...
        # 下采样
        x, skip1 = self.down1(x)  # (batch_size, 128, 128)
        x, skip2 = self.down2(x)  # (batch_size, 256, 64)
        x, skip3 = self.down3(x)  # (batch_size, 512, 32)

        # 中间层
        x = self.mid_conv(x)  # (batch_size, 1024, 32)

        # 上采样
        x = self.up1(x, skip3)  # (batch_size, 512, 64)
        x = self.up2(x, skip2)  # (batch_size, 256, 128)
        x = self.up3(x, skip1)  # (batch_size, 128, 256)
